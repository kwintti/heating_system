...
def add_items(cur, o_date=None, o_temp=None, room_temp=None, eng_price=None, eng_date=None, boi_date=None, boi_on=None, avg_date=None, avg_price=None):
    ...
    if avg_date is not None and avg_price is not None:
        cur.execute("INSERT INTO avg_prices(date, price) VALUES(:avg_date, :avg_price)", {"avg_date": avg_date, "avg_price": avg_price})
...
def init_con(cur):
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS avg_prices(id INTEGER PRIMARY KEY, date DATE, price TEXT)")
...
def check_date_on_last_row(cur, table_name=None):
    ...
    if table_name == "energy_price":
        ...
        date = res.fetchone()[0]
        return date
    if table_name == "avg_prices":
        res = cur.execute("SELECT date FROM avg_prices WHERE id = (SELECT max(id) from avg_prices)") 
...
def get_data(cur, table_name=None, row_nums=None, avg_prices=None):
    if table_name == "out_temp":
    ...
    if table_name == "room_temp":
        ...
        return res.fetchall()
    if table_name == "avg_prices":
        res = cur.execute("SELECT date, price FROM avg_prices WHERE id > ((SELECT max(id) FROM avg_prices) - :row_nums)", {"row_nums": row_nums})
        return res.fetchall()

#  Connecting to our database and dumping data. Queries coming later.
def talk_to_db(o_date=None, o_temp=None, room_temp=None, fetch=False, table_name=None, row_nums=None, getting_data=False, eng_price=None, eng_date=None, boi_date=None, boi_on=None, avg_date=None, avg_price=None, avg_prices=None):
    ...
    with con:
        ...
        add_items(cur, o_date, o_temp, room_temp, eng_price, eng_date, boi_date, boi_on, avg_date, avg_price)
        con.commit()
